...
$(function() {
    $('#addCondition').on('click', function() {
        ...
        conditionRow += '            <option value="8">گروه کاربری ثبت کننده</option>';
        conditionRow += '            <option value="5">فراخوان مرتبط</option>';
        conditionRow += '            <option value="6">مساله مرتبط</option>';
        conditionRow += '            <option value="7">تالار مرتبط</option>';
        conditionRow += '            <option value="9">موضوع مرتبط</option>';
        conditionRow += '        </select>';
        conditionRow += '    </div>';
        ...
        info({report: true, data: getData()}, function(result) {
            ...
            if (result && result.data && result.data.length) {
                for (var index=0,len=result.data.length;index<len;index++) {
                    ...
                    rows += '    <td>' + (row.idea_festival_title ? row.idea_festival_title : '---') + '</td>';
                    rows += '    <td>' + (row.idea_issue_description ? row.idea_issue_description : '---') + '</td>';
